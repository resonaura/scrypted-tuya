  ...
  return QUALITY_CODE_PATTERNS.reduce(
    (score, pattern, index) =>
      pattern.test(normalized)
        ? Math.max(score, QUALITY_CODE_PATTERNS.length - index)
        : score,
    0,
  );
}
...
function chooseSchema(
  schemas: TuyaDeviceSchema[],
): TuyaDeviceSchema | undefined {
  return schemas
    .filter(
      (schema) =>
        schema.type === "Enum" &&
        schema.mode !== "r" &&
        codeScore(schema.code) > 0,
    )
    .filter((schema) =>
      schema.specs.range.some(
        (value: string) => valueScore(value) !== undefined,
      ),
    )
    .sort((a, b) => codeScore(b.code) - codeScore(a.code))[0];
}

export function selectMaximumQuality(
  device: Pick<TuyaDevice, "schema" | "status">,
): TuyaQualitySelection | undefined {
  ...
  const ranked = schema.specs.range
    ...
    .filter(
      (entry): entry is { value: string; score: number } =>
  ...
  return {
    code: schema.code,
    value: best.value,
    current: (device.status || []).find(
      (status) => status.code === schema.code,
    ),
  };
}

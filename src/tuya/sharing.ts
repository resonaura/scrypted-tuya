...
export type TuyaLoginQRCode = { userCode: string } & TuyaResponse<{
  qrcode: string;
}>;
...
export class TuyaSharingAPI {
  ...
  public async fetchDevices(): Promise<TuyaDevice[]> {
    ...
    const devicesResponse = await this._request<TuyaDevice[]>(
      "GET",
      "/v1.0/m/life/ha/home/devices",
      { homeId: firstHomeId },
    );
    return Promise.all(
      (devicesResponse.result || []).map((p) =>
        this.updateDeviceSpecs(p).catch(() => p),
      ),
    );
  }
  ...
  public async fetchMqttConfig(
    ...
    }>("post", "/v1.0/m/life/ha/access/config", undefined, { linkId });
    ...
    return {
      ...
      topics: homeIds
        .map((id) => response.result.topic.ownerId.sub.replace("{ownerId}", id))
        .concat(
          deviceIds.map(
            (id) =>
              response.result.topic.devId.sub.replace("{devId}", id) + "/sta",
          ),
        ),
  ...
  async queryHomes() {
    const response = await this._request<
      [{ uid: string; id: number; ownerId: string; name: string }]
  ...
  private async updateDeviceSpecs(device: TuyaDevice): Promise<TuyaDevice> {
    ...
    try {
      ...
      }>("get", `/v1.1/m/life/${device.id}/specifications`);
      ...
      console.log(
        `[TuyaSharing] Could not fetch specifications for device: ${device.name} [${device.id}]`,
      );
    }
  ...
  private async _request<T = any>(
    ...
    headers.set(
      "X-sign",
      _restfulSign(hashKey, queryEncData, bodyEncData, headers),
    );
    ...
    const response = await this.session.request({
      ...
      data:
        !body || !Object.keys(body).length ? undefined : JSON.stringify(body),
    });

    const ret = response.data
      ? (JSON.parse(response.data) as TuyaResponse<string>)
      : undefined;
    ...
    return {
      ...
      result:
        typeof ret.result == "string"
          ? (JSON.parse(_aesGcmDencrypt(ret.result, secret)) as T)
          : ret.result,
    };
  }
  ...
  private async _internalRefreshTokenIfNeeded() {
    ...
    const response = await this._request<{
      ...
      "GET",
      `/v1.0/m/token/${this.tokenInfo.refreshToken}`,
      undefined,
      undefined,
      true,
    );
  ...
  static async fetchToken(
    qrCodeLogin: TuyaLoginQRCode,
  ): Promise<TuyaSharingTokenInfo> {
    const session = new Axios({});
    const response = await session.request({
      method: "GET",

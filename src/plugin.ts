...
export class TuyaPlugin
  ...
  implements DeviceProvider, Settings
{
  ...
  settingsStorage = new StorageSettings(this, {
    ...
    userCode: {
      title: "User Code",
      description:
        "Required: You can find this information in Tuya (Smart Life) App -> Settings -> Account and Security -> User Code.",
      onPut: () => this.tryLogin(),
    },
    qrCode: {
      title: "Login QR Code",
      ...
      mapGet(value) {
        if (value) {
          return new QRCode(
            `tuyaSmart--qrLogin?token=${(value as TuyaLoginQRCode).result.qrcode}`,
          ).svg({ container: "svg" });
        } else {
  ...
  async getSettings(): Promise<Setting[]> {
    ...
    this.settingsStorage.settings.userCode.hide =
      loginMethod != TuyaLoginMethod.App;
    this.settingsStorage.settings.qrCode.hide =
      loginMethod != TuyaLoginMethod.App ||
      !userCode ||
      !!this.settingsStorage.values.tokenInfo;
    this.settingsStorage.settings.qrCodeLoggedIn.hide =
      this.settingsStorage.settings.qrCode.hide;

    // Show old login method
    this.settingsStorage.settings.userId.hide =
      loginMethod != TuyaLoginMethod.Account;
    this.settingsStorage.settings.accessId.hide =
      loginMethod != TuyaLoginMethod.Account;
    this.settingsStorage.settings.accessKey.hide =
      loginMethod != TuyaLoginMethod.Account;
    this.settingsStorage.settings.country.hide =
      loginMethod != TuyaLoginMethod.Account;
  ...
  async releaseDevice(id: string, nativeId: ScryptedNativeId): Promise<void> {
    this.console.info(
      `[${this.name}] (${new Date().toLocaleString()}) Device with id '${nativeId}' was removed.`,
    );
  }

  private async tryLogin(
    state: { useTokenFromStorage?: boolean; loggedInClicked?: boolean } = {},
  ) {
    this.api = undefined;
    this.mq = undefined;
    ...
    let storeToken: TuyaTokenInfo | undefined = useTokenFromStorage
      ? this.settingsStorage.values.tokenInfo
      : undefined;
    ...
    if (!storeToken) {
      ...
      switch (method) {
        case TuyaLoginMethod.App:
          const userCode = this.settingsStorage.values.userCode;
          const qrCodeValue = this.settingsStorage.settings.qrCode
            .defaultValue as TuyaLoginQRCode | undefined;
          if (!userCode) {
            ...
            try {
              ...
              this.console.log(
                `[${this.name}] (${new Date().toLocaleString()}) Failed to fetch new QR Code.`,
                e,
              );
            }
            this.onDeviceEvent(ScryptedInterface.Settings, undefined);
            ...
            try {
              ...
              this.console.log(
                `[${this.name}] (${new Date().toLocaleString()}) Failed to authenticate with QR Code.`,
                e,
              );
              this.log.a(
                "Failed to authenticate with credentials. Ensure you scanned the QR Code with Tuya (Smart Life) App and try again.",
              );
            }
            this.onDeviceEvent(ScryptedInterface.Settings, undefined);
          }
        ...
        case TuyaLoginMethod.Account:
          try {
            ...
            this.console.log(
              `[${this.name}] (${new Date().toLocaleString()}) Failed to authenticate.`,
              e,
            );
      ...
      this.console.info(
        `[${this.name}] (${new Date().toLocaleString()}) Using stored token for login.`,
      );
    }

    this.settingsStorage.putSetting(
      "tokenInfo",
      storeToken ? JSON.stringify(storeToken) : undefined,
    );
      ...
      case TuyaLoginMethod.App:
        this.api = new TuyaSharingAPI(
          ...
          (updatedToken) => {
            this.settingsStorage.putSetting(
              "tokenInfo",
              JSON.stringify({ ...updatedToken, type: TuyaLoginMethod.App }),
            );
          },
          () => {
            this.settingsStorage.putSetting("tokenInfo", undefined);
            this.console.log(
              `[${this.name}] (${new Date().toLocaleString()}) Request reauthentication.`,
            );
            this.onDeviceEvent(ScryptedInterface.Settings, undefined);
            this.log.a(
              `Reauthentication to Tuya required. Refresh plugin to retrieve new QR Code.`,
            );
          },
        );
        break;
      case TuyaLoginMethod.Account:
        this.api = new TuyaCloudAPI(
          ...
          () => {
            ...
            this.log.a(
              `Reauthentication to Tuya required. Refresh plugin to log in again.`,
            );
          },
        );
    }
    ...
    try {
      if (this.api instanceof TuyaSharingAPI) {
        ...
        const fetch = async function () {
          ...
          return await api.fetchMqttConfig(
            homes.map((h) => h.ownerId),
            devices.map((d) => d.id),
          );
        };
        ...
        this.mq.on("error", (error) => {
          this.console.error(
            `[${this.name}] (${new Date().toLocaleString()}) failed to connect to mqtt, will retry.`,
            error,
          );
        });
        await this.mq.start();
      ...
      this.console.log(
        `[${this.name}] (${new Date().toLocaleString()}) Failed to connect to Mqtt. Will not observe live changes to devices.`,
      );
    }
  }
  ...
  private onMessage(message: TuyaMessage) {
    ...
    if (message.protocol === TuyaMessageProtocol.DEVICE) {
      ...
      if (!device) return;
      if (
        message.data.bizCode === "online" ||
        message.data.bizCode === "offline"
      ) {
